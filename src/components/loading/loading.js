...
// Función para crear el skeleton completo
export const createSkeleton = () => {
  const app = document.getElementById('app');
  
  // Asegurarnos de no crear otro skeleton si ya existe
  let skeletonContainer = document.getElementById('skeleton-container');
  if (!skeletonContainer) {
    skeletonContainer = document.createElement('div');
    skeletonContainer.id = 'skeleton-container'; // Añadir un id para poder eliminarlo después
    skeletonContainer.classList.add('skeleton-container');
    
    // Crear diferentes tipos de skeletons (avatar, imagen, texto)
    for (let i = 0; i < 5; i++) {
